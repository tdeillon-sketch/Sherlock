...
export interface DebutantQuestion {
  type: number;
  typeDesc: string;
...
export interface ConnaisseurQuestion {
...
export interface ExpertQuestion {
  clue: string; // Very short behavioral hint
  correctType: number;
  correctCelebrity: string;
  celebrityOptions: string[]; // 3-4 options
  explanation: string;
}

export type GameLevel = 'debutant' | 'connaisseur' | 'expert';

export interface GameLevelConfig {
  key: GameLevel;
  title: string;
  emoji: string;
  badge: string; // Badge emoji earned on completion
  desc: string;
  requiredScore: number; // minimum score to unlock badge (out of total questions)
}

export const GAME_LEVELS: GameLevelConfig[] = [
  { key: 'debutant', title: 'Debutant', emoji: '\u{1F331}', badge: '\u{1F949}', desc: 'On vous donne le type, trouvez la personnalite', requiredScore: 6 },
  { key: 'connaisseur', title: 'Connaisseur', emoji: '\u{1F50D}', badge: '\u{1F948}', desc: 'Lisez la description, devinez le type', requiredScore: 7 },
  { key: 'expert', title: 'Expert', emoji: '\u{1F3AF}', badge: '\u{1F947}', desc: 'Devinez le type ET la personnalite', requiredScore: 8 },
];
...
// ===== QUIZ DEBUTANT (9 questions) =====
// On donne le type, trouvez la personnalite parmi 3 options

export const QUIZ_DEBUTANT: DebutantQuestion[] = [
  {
    type: 1,
...
// ===== QUIZ CONNAISSEUR (12 questions) =====
// Deviner le type a partir d'une description comportementale

export const QUIZ_CONNAISSEUR: ConnaisseurQuestion[] = [
  {
...
// ===== QUIZ EXPERT (10 questions) =====
// Indice court -> deviner le type ET la personnalite

export const QUIZ_EXPERT: ExpertQuestion[] = [
  {
    clue: "Cette personne s'imposait le jeune et le silence comme preuves de coherence morale, marchant des milliers de kilometres pour reformer un pays entier.",
    correctType: 1,
    correctCelebrity: 'Mahatma Gandhi',
    celebrityOptions: ['Mahatma Gandhi', 'Martin Luther King Jr.', 'Desmond Tutu'],
    explanation:
      "Gandhi (type 1, Perfectionniste) canalisait sa colere en reforme du monde, avec une discipline de vie extreme et un sens moral inflexible.",
  },
  {
    clue: "Ce religieux rayonnait de chaleur humaine et repondait a la haine par le pardon, cherchant toujours a guerir les blessures des autres avant les siennes.",
    correctType: 2,
    correctCelebrity: 'Desmond Tutu',
    celebrityOptions: ['Desmond Tutu', 'Bob Marley', 'Mahatma Gandhi', 'Mister Rogers'],
    explanation:
      "Desmond Tutu (type 2, Genereux) guidait sa lutte contre l'apartheid par la compassion et le pardon, incarnant l'empathie naturelle du type 2.",
  },
  {
    clue: "Cette artiste adapte minutieusement son image a chaque nouvelle ere de sa carriere, transformant meme ses echecs en victoires publiques strategiques.",
    correctType: 3,
    correctCelebrity: 'Taylor Swift',
    celebrityOptions: ['Taylor Swift', 'Beyonce', 'Billie Eilish'],
    explanation:
      "Taylor Swift (type 3, Gagneur) orchestre sa carriere avec une precision remarquable, convertissant l'authenticite en triomphe commercial.",
  },
  {
    clue: "Cet acteur choisit systematiquement des roles etranges et marginaux, cherchant la beaute dans l'excentricite et transformant sa singularite en art.",
    correctType: 4,
    correctCelebrity: 'Johnny Depp',
    celebrityOptions: ['Johnny Depp', 'Jim Carrey', 'Keanu Reeves', 'Robin Williams'],
    explanation:
      "Johnny Depp (type 4, Artiste) est attire par les personnages incompris et poetiques, refletant son propre sentiment d'etre different.",
  },
  {
    clue: "Ce scientifique vivait dans le monde des equations malgre un corps immobilise, explorant les confins du cosmos avec une liberte mentale totale.",
    correctType: 5,
    correctCelebrity: 'Stephen Hawking',
    celebrityOptions: ['Stephen Hawking', 'Albert Einstein', 'Bill Gates'],
    explanation:
      "Stephen Hawking (type 5, Observateur) incarnait le detachement intellectuel — les limites physiques ne comptaient pas tant que l'esprit restait libre.",
  },
  {
    clue: "Cette actrice est connue pour sa fidelite indefectible a ses amis proches et sa constance, cultivant la securite emotionnelle comme priorite absolue.",
    correctType: 6,
    correctCelebrity: 'Jennifer Aniston',
    celebrityOptions: ['Jennifer Aniston', 'Michelle Obama', 'Princesse Diana'],
    explanation:
      "Jennifer Aniston (type 6, Loyal) incarne la fidelite et le besoin de stabilite, loin des excentricites hollywoodiennes.",
  },
  {
    clue: "Cet entrepreneur collectionne les aventures extremes — traversees en ballon, voyages spatiaux — refusant toute routine et transformant chaque echec en opportunite.",
    correctType: 7,
    correctCelebrity: 'Richard Branson',
    celebrityOptions: ['Richard Branson', 'Mark Zuckerberg', 'Bill Gates'],
    explanation:
      "Richard Branson (type 7, Aventurier) vit comme si la vie etait un buffet, avec un optimisme et une soif de nouveaute indestructibles.",
  },
  {
    clue: "Cette championne domine par sa puissance brute et transforme sa colere en carburant sur le terrain, luttant contre les prejuges avec feroce determination.",
    correctType: 8,
    correctCelebrity: 'Serena Williams',
    celebrityOptions: ['Serena Williams', 'Cristiano Ronaldo', 'Beyonce'],
    explanation:
      "Serena Williams (type 8, Chef) refuse de se laisser diminuer et transforme la colere en force — le moteur inepuisable du type 8.",
  },
  {
    clue: "Cet acteur traverse l'industrie du cinema sans ego apparent, partageant ses cachets avec les equipes techniques et cultivant une paix interieure remarquable.",
    correctType: 9,
    correctCelebrity: 'Keanu Reeves',
    celebrityOptions: ['Keanu Reeves', 'Morgan Freeman', 'Bob Marley'],
    explanation:
      "Keanu Reeves (type 9, Mediateur) cherche l'harmonie partout, evitant les conflits et restant d'une humilite desarmante.",
  },
  {
    clue: "Ce personnage fictif protege les siens avec feroce determination dans une arene mortelle, refusant de se soumettre au pouvoir injuste d'un regime totalitaire.",
    correctType: 8,
    correctCelebrity: 'Katniss Everdeen',
    celebrityOptions: ['Katniss Everdeen', 'Hermione Granger', 'Bilbo Sacquet', 'Peter Pan'],
    explanation:
      "Katniss Everdeen (type 8, Chef) est un Chef instinctif dont la force vient de la protection des siens, pas de la domination.",
  },
];
